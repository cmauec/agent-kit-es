...
- **`StreamEvent`:** solo se emite cuando los mensajes parciales están habilitados. Contiene eventos de streaming sin procesar de la API (deltas de texto, fragmentos de entrada de herramientas). Consulta [Stream responses](/docs/en/agent-sdk/streaming-output).
...
- **Streaming en vivo:** habilita los mensajes parciales (`include_partial_messages` en Python, `includePartialMessages` en TypeScript) para recibir mensajes `StreamEvent` en tiempo real. Consulta [Stream responses in real-time](/docs/en/agent-sdk/streaming-output).
...
- **Conectar servicios externos** con [servidores MCP](/docs/en/agent-sdk/mcp) (bases de datos, navegadores, APIs)
- **Definir herramientas personalizadas** con [manejadores de herramientas personalizadas](/docs/en/agent-sdk/custom-tools)
- **Cargar skills de proyecto** mediante [fuentes de configuración](/docs/en/agent-sdk/claude-code-features) para flujos de trabajo reutilizables
...
- **`disallowed_tools` / `disallowedTools`** bloquea las herramientas listadas, independientemente de otras configuraciones. Consulta [Permissions](/docs/en/agent-sdk/permissions) para ver el orden en que se verifican las reglas antes de que se ejecute una herramienta.
...
También puedes delimitar herramientas individuales con reglas como `"Bash(npm:*)"` para permitir solo comandos específicos. Consulta [Permissions](/docs/en/agent-sdk/permissions) para la sintaxis completa de reglas.
...
Puedes limitar cuántos turnos toma el bucle, cuánto cuesta, qué tan profundo razona Claude y si las herramientas requieren aprobación antes de ejecutarse. Todos estos son campos en [`ClaudeAgentOptions`](/docs/en/agent-sdk/python#claude-agent-options) (Python) / [`Options`](/docs/en/agent-sdk/typescript#options) (TypeScript).
...
Cuando se alcanza cualquiera de los límites, el SDK devuelve un `ResultMessage` con un subtipo de error correspondiente (`error_max_turns` o `error_max_budget_usd`). Consulta [Manejar el resultado](#manejar-el-resultado) para saber cómo verificar estos subtipos, y [`ClaudeAgentOptions`](/docs/en/agent-sdk/python#claude-agent-options) / [`Options`](/docs/en/agent-sdk/typescript#options) para la sintaxis.
...
| **Archivos CLAUDE.md** | Inicio de sesión, cuando [`settingSources`](/docs/en/agent-sdk/claude-code-features) está habilitado | Contenido completo en cada solicitud (pero con prompt cache, solo la primera solicitud paga el costo completo) |
| **Definiciones de herramientas** | Cada solicitud | Cada herramienta agrega su esquema; usa [MCP tool search](/docs/en/agent-sdk/mcp#mcp-tool-search) para cargar herramientas bajo demanda en lugar de todas a la vez |
...
- **Hook `PreCompact`:** Ejecuta lógica personalizada antes de que ocurra la compactación, por ejemplo para archivar la transcripción completa. El hook recibe un campo `trigger` (`manual` o `auto`). Consulta [hooks](/docs/en/agent-sdk/hooks).
- **Compactación manual:** Envía `/compact` como cadena de prompt para activar la compactación bajo demanda. (Los slash commands enviados de esta manera son entradas del SDK, no atajos exclusivos del CLI. Consulta [slash commands en el SDK](/docs/en/agent-sdk/slash-commands).)
...
Consulta [Session management](/docs/en/agent-sdk/sessions) para la guía completa sobre los patrones de reanudación, continuación y bifurcación.
...
Los [Hooks](/docs/en/agent-sdk/hooks) son callbacks que se disparan en puntos específicos del bucle: antes de que se ejecute una herramienta, después de que devuelva un resultado, cuando el agente termina, etc. Algunos hooks de uso común son:
...
Ambos SDKs admiten todos los eventos anteriores. El SDK de TypeScript incluye eventos adicionales que Python aún no admite. Consulta [Control execution with hooks](/docs/en/agent-sdk/hooks) para la lista completa de eventos, disponibilidad por SDK y la API completa de callbacks.
...
- **¿Aún no has ejecutado un agente?** Comienza con el [quickstart](/docs/en/agent-sdk/quickstart) para instalar el SDK y ver un ejemplo completo en funcionamiento de principio a fin.
- **¿Listo para integrarlo en tu proyecto?** [Carga CLAUDE.md, skills y filesystem hooks](/docs/en/agent-sdk/claude-code-features) para que el agente siga las convenciones de tu proyecto automáticamente.
- **¿Construyendo una interfaz interactiva?** Habilita el [streaming](/docs/en/agent-sdk/streaming-output) para mostrar texto en vivo y llamadas a herramientas mientras se ejecuta el bucle.
- **¿Necesitas un control más estricto sobre lo que puede hacer el agente?** Restringe el acceso a herramientas con [permissions](/docs/en/agent-sdk/permissions), y usa [hooks](/docs/en/agent-sdk/hooks) para auditar, bloquear o transformar llamadas a herramientas antes de que se ejecuten.
- **¿Ejecutando tareas largas o costosas?** Delega trabajo aislado a [subagentes](/docs/en/agent-sdk/subagents) para mantener tu contexto principal liviano.
